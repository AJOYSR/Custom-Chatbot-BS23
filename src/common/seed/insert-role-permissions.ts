    ...
    for (const rolePermission of role_permissions) {
      ...
      if (!existingByExactMatch) {
        ...
        if (!existingByRelationship) {
          ...
          console.log(
            `Skip: Role-permission relationship already exists for roleId: ${rolePermission.roleId} and permissionId: ${rolePermission.permissionId}`
          );
        }
      } else {
        console.log(
          `Skip: Role-permission with ID ${rolePermission._id} already exists`
        );
      }
    }
